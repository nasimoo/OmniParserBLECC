...
def get_parsed_content_icon(filtered_boxes, starting_idx, image_source, caption_model_processor, prompt=None, batch_size=128):
    ...
    for i in range(0, len(croped_pil_image), batch_size):
        ...
        # Convert input_ids and attention_mask to long (int64) regardless of device
        for key in ['input_ids', 'attention_mask']:
            if key in inputs:
                inputs[key] = inputs[key].long()
        
        # Move tensors to device and convert pixel_values to model's dtype
        inputs = {
            k: (v.to(device=device) if k in ['input_ids', 'attention_mask'] else 
                v.to(device=device, dtype=dtype) if torch.is_tensor(v) else v)
